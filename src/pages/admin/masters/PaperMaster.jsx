...
export default function PaperMaster() {
  ...
  const [formValues, setFormValues] = useState({ code: '', name: '', gsm: '' });
  ...
  const openEditDialog = (row) => {
    ...
    setFormValues({
      code: row.code || '',
      name: row.name || '',
      gsm: row.gsm != null ? String(row.gsm) : ''
    });
    setError('');
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const gsm = formValues.gsm.trim() ? Number(formValues.gsm.trim()) : undefined;
    ...
    if (gsm !== undefined && Number.isNaN(gsm)) {
      setError('GSM must be a valid number');
      return;
    }
    ...
    try {
      if (editingRow && editingRow.id) {
        await editPaper(editingRow.id, { code, name, gsm });
        await loadPapers();
      } else {
        await createPaper({ code, name, gsm });
        await loadPapers();
      }
  ...
  const handleToggleActive = async (row, active) => {
    ...
    try {
      await togglePaperActive(row.id, active);
    } catch (e) {
  ...
  return (
    <>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="xs" fullWidth>
        <DialogTitle>{editingRow ? 'Edit Paper Type' : 'Create Paper Type'}</DialogTitle>
