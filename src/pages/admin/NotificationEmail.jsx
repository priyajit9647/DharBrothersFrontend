...
import { fetchEmailNotificationHistory } from 'api/emailApi';

// ==============================|| BMS - EMAIL NOTIFICATIONS ||============================== //
...
export default function NotificationEmail() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      const data = await fetchEmailNotificationHistory();
      const items = Array.isArray(data) ? data : data?.items || data?.data || [];
      setRows(items);
    } catch (err) {
      console.error('Failed to load email notifications', err);
      setError(err?.message || 'Failed to load email notifications');
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <Grid item xs={12} sx={{ width: '100%', flexGrow: 1 }}>
          ...
          <MasterList
            ...
            columns={[
              ...
              { id: 'createdDate', label: 'Created Date', format: (value) => (value ? new Date(value).toLocaleString() : '') }
            ]}
            rows={pagedRows}
            page={page}
